...
class MobileFacenet(SnpeContext):
    ...
    def get_embedding(self, face_image):
        ...
        if not self.Execute():
            print("Error: Failed to execute MobileFacenet model")
            return None
    ...
    def compare_faces(embedding1, embedding2, threshold=0.4):
        similarity = MobileFacenet.cosine_similarity(embedding1, embedding2)
        distance = MobileFacenet.euclidean_distance(embedding1, embedding2)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='MobileFacenet Face Recognition Example')
    ...
    parser.add_argument('--dlc', type=str, default='mobilefacenet_quantized_6490.dlc',
                       help='Path to MobileFacenet DLC model')
    ...
    if not mobilefacenet.Initialize():
        print("Error: Failed to initialize MobileFacenet model!")
        exit(1)
    ...
    embedding1 = mobilefacenet.get_embedding(image1)
    ...
    if args.image2:
        ...
        embedding2 = mobilefacenet.get_embedding(image2)
        ...
        result = MobileFacenet.compare_faces(embedding1, embedding2)
